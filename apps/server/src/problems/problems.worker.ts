    ...
    if (msg !== null) {
      ...
      try {
        ...
        if (!testRun && !submission) {
          logger.warn(
            `Processing result for ${isTest ? "test run" : "submission"} ${submissionId} but record not found in DB. Skipping.`
          )
        ...
        if (effectiveIsTest) {
          await client.query("UPDATE test_run SET status = $1 WHERE id = $2", [
            "PENDING",
            recordId,
          ])
        } else {
        ...
        if (effectiveIsTest) {
          const countRes = await client.query(
            'SELECT COUNT(*)::int FROM test_run_result WHERE "testRunId" = $1',
            [recordId]
          )
          resultsCount = countRes.rows[0].count
        } else {
          const countRes = await client.query(
            'SELECT COUNT(*)::int FROM test_case_result WHERE "submissionId" = $1',
            [recordId]
          )
          resultsCount = countRes.rows[0].count
        }
        ...
        if (resultsCount === totalCases) {
          ...
          if (effectiveIsTest) {
            const resultsRes = await client.query(
              'SELECT * FROM test_run_result WHERE "testRunId" = $1 ORDER BY "caseIdx" ASC',
              [recordId]
            )
            allResults = resultsRes.rows
          } else {
            const resultsRes = await client.query(
              'SELECT * FROM test_case_result WHERE "submissionId" = $1 ORDER BY "caseIdx" ASC',
              [recordId]
            )
            allResults = resultsRes.rows
          }
          ...
          const maxMemory = Math.max(
            ...allResults.map((r: any) => r.memory || 0)
          )
          ...
          if (effectiveIsTest) {
            ...
            if (!anyFailed && updatedSubmission.contestId) {
              ...
              if (contestProblem) {
                ...
                if (
                  !currentProgress ||
                  currentProgress.currentOrder < newProgressOrder
                ) {
                  await client.query(
                  ...
                  logger.info(
                    `Advanced contest progress for user ${updatedSubmission.userId} in contest ${updatedSubmission.contestId} to problem ${newProgressOrder}`
                  )
                }
              }
            }
          ...
          logger.info(
            `Completed ${effectiveIsTest ? "test run" : "submission"} ${recordId}. Status: ${finalStatus}`
          )
        }

        logger.info(
          `Processed test case ${caseIdx + 1}/${totalCases} for ${effectiveIsTest ? "test run" : "submission"} ${recordId}. Status: ${status}`
        )
